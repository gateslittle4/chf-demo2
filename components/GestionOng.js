...
// Gestion de la liste des ONG partenaires (Firestore, collection "ong_partenaires").
...
function GestionOngPanel({ listeOngDocs, showToast }) {
  ...
  const ajouter = async () => {
    ...
    if (!nom) { showToast("Entrez un nom d'ONG.", "error"); return; }
    if (listeOngDocs.some(o => o.nom.toLowerCase() === nom.toLowerCase())) { showToast("Cette ONG existe déjà.", "error"); return; }
    try {
      ...
      setNouveauNom("");
      showToast("ONG ajoutée avec succès", "success");
  ...
  return (
    <div className="space-y-4 text-xs">
      <div className="bg-white p-4 rounded-xl border shadow-sm space-y-2">
        <h2 className="font-black text-gray-800 mb-1">🤝 ONG partenaires</h2>
        <p className="text-gray-500">Ajoute une ONG pour qu'elle apparaisse dans les listes de sélection (nouveau dossier, factures, archives, caisse).</p>
        <div className="flex gap-2">
          <input
            type="text" value={nouveauNom} onChange={e => setNouveauNom(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') ajouter(); }}
            placeholder="Nom de l'ONG..." className="border rounded-lg p-2 flex-1 outline-none"
          />
      ...
      <div className="bg-white rounded-xl border overflow-hidden divide-y">
        {listeOngDocs.length === 0 && <p className="text-gray-500 p-3">Aucune ONG enregistrée.</p>}
        {listeOngDocs.map(o => (
