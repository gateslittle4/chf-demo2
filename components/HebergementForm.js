...
function HebergementForm({
  ...
  hasChirSpec, setHasChirSpec, nomChirSpec, setNomChirSpec, prixChirSpec, setPrixChirSpec
}) {
  return (
    <div className="bg-white p-4 rounded-xl border space-y-3 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div><label className="text-[10px] font-bold text-gray-500 uppercase">Type de lit</label><select value={typeLit1} onChange={e=>setTypeLit1(e.target.value)} className="border rounded-lg p-1.5 bg-white text-xs w-full"><option value="normal">Lit normal (500 Gdes/j)</option><option value="semi_prive">Salle Semi Privé (500)</option><option value="prive">Salle Privé (1500)</option><option value="isolette">Isolette (1250)</option><option value="incubateur">Incubateur (2500)</option></select></div>
      </div>
