...
// utils/constants n'est plus requis directement ici — la liste des partenaires vient en prop (listeOng).
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 border-b pb-2">
        ...
        <button onClick={()=>setSousOnglet("rapport_ong")} className={`px-4 py-2 font-bold text-xs rounded-t-lg ${sousOnglet==="rapport_ong" ? "bg-[#1E2A24] text-white" : "bg-gray-100"}`}>📊 Rapport Partenaire</button>
      </div>
      <div className="bg-white p-3 rounded-xl border shadow-sm grid grid-cols-1 sm:grid-cols-4 gap-2 text-xs">
        ...
        <div><label className="text-[10px] font-bold text-gray-400">Mode</label>
          <select value={filtreMode} onChange={e=>setFiltreMode(e.target.value)} className="border rounded p-1.5 w-full bg-white">
            <option value="">Tous</option><option value="cash">Cash</option><option value="credit">Crédit</option><option value="ong">Partenaire</option><option value="exoneration">Exonération</option><option value="depot">Dépôt</option>
          </select>
        </div>
        <div><label className="text-[10px] font-bold text-gray-400">Partenaire</label>
          ...
          <div className="bg-white p-4 rounded-xl border shadow-sm">
            ...
            <div className="overflow-x-auto max-h-64 overflow-y-auto">
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-gray-100"><tr><th>Patient</th><th>Montant</th><th>Mode</th><th>Partenaire</th><th>Date</th></tr></thead>
                <tbody>
                  {paiementsFiltres.slice(0, 50).map((p, i) => (
                    <tr key={i} className="border-b"><td className="p-1">{p.patientNom}</td><td className="p-1">{formatGourdes(p.montant)}</td><td className="p-1">{LABELS_MODE[p.mode] || p.mode}</td><td className="p-1">{p.ongPartenaire || '—'}</td><td className="p-1">{new Date(p.date).toLocaleDateString('fr-FR')}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
      ...
      {sousOnglet === "rapport_ong" && (
        <div className="bg-white p-4 rounded-xl border shadow-sm space-y-3">
          <div className="flex justify-between items-center"><h3 className="font-bold text-gray-700 text-xs uppercase">📊 Rapport croisé Partenaire / Mode</h3><button onClick={imprimerRapport} className="bg-gray-700 text-white px-2 py-1 rounded text-[10px] font-bold flex items-center gap-1"><Printer size={12}/> Imprimer</button></div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead><tr className="bg-gray-100"><th className="p-2 border">Partenaire</th>{rapportONG.modes.map(m => <th key={m} className="p-2 border text-center">{LABELS_MODE[m] || m.toUpperCase()}</th>)}<th className="p-2 border text-center">Total</th></tr></thead>
              <tbody>
