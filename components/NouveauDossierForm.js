    ...
    <div className="bg-white p-6 rounded-xl border shadow-sm space-y-4">
      ...
      {peutCreerDossier ? (
        <form onSubmit={onSoumettre} className="space-y-3">
          ...
          <div className="flex flex-col gap-1"><label className="text-[11px] font-bold uppercase text-gray-400">Numéro de dossier</label><input type="text" value={inputNumDossier} onChange={e=>setInputNumDossier(e.target.value)} placeholder="Laisse vide pour un numéro auto (N-1, N-2...)" className="border rounded-lg p-2 text-xs outline-none" /></div>
