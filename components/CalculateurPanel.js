...
function CalculateurPanel({
  ...
  const genererCorpsTicket = (fiche, { afficherTotalDossier, totalDossier } = {}) => {
    ...
    const ligneHebergement2 = fiche.exeat?.multiPeriode && fiche.exeat?.dateEntree2 ? `<tr><td>Séjour P2 : ${echapperHTML(CONFIG_LITS[fiche.exeat.typeLit2]?.nom || fiche.exeat.typeLit2)} (${formaterPlageDates(fiche.exeat.dateEntree2, fiche.exeat.dateSortie2, true)})</td><td class="qte">${fiche.exeat.nbJours2}j</td><td class="prix">${formatGourdes(fiche.exeat.prixParJour2 ?? (CONFIG_LITS[fiche.exeat.typeLit2]?.prix || 0))}</td><td class="sous-total">${formatGourdes(fiche.exeat.totalHebergement2)}</td></tr>` : '';
  ...
  const reimprimerFicheValidee = (fiche) => {
    ...
    const contenu = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Fiche N°${fiche.numeroFiche}</title><style>${STYLE_TICKET}</style></head><body>${genererCorpsTicket(fiche, { afficherTotalDossier: estLaDerniereFiche, totalDossier: totalDossierGourdes })}</body></html>`;
  ...
  const imprimerFicheA4 = () => {
    ...
    const ligneHebergement = data.dateEntree1 && data.dateSortie1 ? `<tr><td>Hébergement — ${echapperHTML(CONFIG_LITS[data.typeLit1].nom)}</td><td class="qte">${data.j1} j</td><td class="prix">${formatGourdes(prixLit(data.typeLit1, tarifChoisi))}</td><td class="mtotal">${formatGourdes(data.totalE1)}</td></tr>` : '';
    const ligneHebergement2 = data.multiPeriode && data.dateEntree2 && data.dateSortie2 ? `<tr><td>Hébergement (2e période) — ${echapperHTML(CONFIG_LITS[data.typeLit2].nom)}</td><td class="qte">${data.j2} j</td><td class="prix">${formatGourdes(prixLit(data.typeLit2, tarifChoisi))}</td><td class="mtotal">${formatGourdes(data.totalE2)}</td></tr>` : '';
