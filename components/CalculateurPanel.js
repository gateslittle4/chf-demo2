...
function CalculateurPanel({
  ...
  const genererCorpsTicket = (fiche) => {
    ...
    const ligneHebergement = fiche.exeat ? `<tr><td>Séjour : ${echapperHTML(CONFIG_LITS[fiche.exeat.typeLit]?.nom || fiche.exeat.typeLit)}</td><td class="qte">${fiche.exeat.nbJours}j</td><td class="prix">${formatGourdes(fiche.exeat.prixParJour)}</td><td class="sous-total">${formatGourdes(fiche.exeat.totalHebergement)}</td></tr>` : '';
