      ...
      function HistoriqueVerifPanel({ verifications, setVerifications, onChargerPourModif, onSupprimer, filtreInitialNom, clearFiltreInitialNom, dossierPourExport, setDossierPourExport, userRole, showToast, onChangerTypeOng }) {
        ...
        const exporterBlocDossiersExcelPartenaire = async (ongCible) => {
          try {
            ...
            const derniereCol = colonnesExport.length + 4;
            let r = 1;
            ...
            ws.getCell(r, 3).value = `#${Math.floor(Math.random() * 1e4)}`;
            ...
            ws.getCell(r, 2).value = "Nom et Prenom";
            appliquerStyle(ws.getCell(r, 3), EXCEL_STYLES.teteColonne);
            ...
            {
              const cell = ws.getCell(r, 4 + colonnesExport.length);
              cell.value = "Total";
              appliquerStyle(cell, EXCEL_STYLES.teteColonne);
            }
            r++;
            ...
            listeDossiersONG.forEach((doc) => {
              ...
              ws.getCell(r, 2).value = doc.nomPatient;
              appliquerStyle(ws.getCell(r, 3), EXCEL_STYLES.celluleStandard);
              ws.getCell(r, 3).value = doc.periodeSejourString || doc.dateHeure || "\u2014";
              colonnesExport.forEach((c, i) => {
            ...
            if (rabaisVal > 0) {
              appliquerStyle(ws.getCell(r, 2), EXCEL_STYLES.gras);
              ws.getCell(r, 2).value = "R\xE9ductions 10%";
              const cell = ws.getCell(r, 4 + colonnesExport.length);
            ...
            if (donsVal > 0) {
              appliquerStyle(ws.getCell(r, 2), EXCEL_STYLES.gras);
              ws.getCell(r, 2).value = "Dons / Intrants";
              const cell = ws.getCell(r, 4 + colonnesExport.length);
              cell.value = donsVal;
              appliquerStyle(cell, EXCEL_STYLES.celluleTotal);
              r++;
            ...
            if (rabaisVal > 0 || donsVal > 0) {
              ...
              ws.getCell(r, 2).value = "MONTANT NET D\xDB";
